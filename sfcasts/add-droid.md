...
You might be asking yourself: "Hold up, do I really need to create a
`StarshipDroid` object every time I want to simply assign a droid to a starship?"
...
The first thing on our list is to
check if the `Starship` already has the droid in question. To
make this happen, switch the property to use the `getDroids()`
...
## Cascding the Persist

Ah ha, a new error. This one is pretty common in Doctrine,
