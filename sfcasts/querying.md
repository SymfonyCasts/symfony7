...
Back in the template, add `|replace({'_': ' '})|title`. This replaces underscores
with spaces and uppercases the first letter of each word.
...
`$this->getEntityManager()->getClassMetadata(Scout::class)`. This returns a special
...
Try switching the class to `Freighter::class` and refresh the homepage. We now
