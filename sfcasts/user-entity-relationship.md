...
`$myShip = $repository->findMyShip()`. If we jump into this method, we can see
the code is just grabbing the first ship in the database.

Wouldn't it be great if this was actually tied to the user?

First, back in `MainController`, set `$myShip` to `null`. This simulates if
there is no logged-in user, or, the current user doesn't have a starship.
...
set `$myShip` to `$this->getUser()?->getStarship()`. The `getUser()` method is another
helper provided by the `AbstractController`. It returns the currently logged-in user
or `null` if there isn't one. `?->` is the null-safe operator, so if `getUser()` returns `null`,
it won't try to call `getStarship()`. It'll just return `null`.
