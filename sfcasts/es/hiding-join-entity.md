...
Utiliza `$this->starshipDroids->map()` para transformar cada elemento de la colección`StarshipDroid` en un objeto `Droid`. Ahora tenemos un método `getDroids()` que devuelve de nuevo una colección de objetos`Droid`. ¡Genial!
...
Vuelve a la plantilla de la página de inicio y actualízala. ¡Lo hemos conseguido! Obtener los droides de una nave sigue siendo fácil. Y el resto de nuestro código no ha tenido que cambiar.
