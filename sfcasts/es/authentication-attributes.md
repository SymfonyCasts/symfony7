...
Security. Estoy en PhpStorm, así que usaré la tecla «Shift» dos veces para activar
...
se determinan en tiempo de ejecución en función del estado actual de inicio de sesión. Nos indican si el
...
pero se usan de la misma manera, así que primero comprobaremos si tiene el rol « `ROLE_USER` ».

El método `isGranted()` está disponible porque nuestra clase de controlador hereda de `AbstractController`.
Esta clase base nos proporciona un montón de métodos útiles, incluidos los ayudantes de seguridad.

Siguiente punto: ` `AuthenticatedVoter::IS_AUTHENTICATED_FULLY => $this->isGranted(AuthenticatedVoter::IS_AUTHENTICATED_FULLY)``.
Aquí estoy usando las constantes, pero usar los valores de cadena sin formato también funcionaría igual de bien.

Luego, ` `AuthenticatedVoter::IS_AUTHENTICATED_REMEMBERED => $this->isGranted(AuthenticatedVoter::IS_AUTHENTICATED_REMEMBERED)``,
``AuthenticatedVoter::IS_AUTHENTICATED => $this->isGranted(AuthenticatedVoter::IS_AUTHENTICATED)``,
``AuthenticatedVoter::IS_REMEMBERED => $this->isGranted(AuthenticatedVoter::IS_REMEMBERED)`` y, por último,
``AuthenticatedVoter::PUBLIC_ACCESS => $this->isGranted(AuthenticatedVoter::PUBLIC_ACCESS)``:

[[[ code('acf64538a7') ]]]

¿Los he incluido todos? 1, 2, 3, 4, 5, y en `AuthenticatedVoter`... 1, 2, 3, 4, 5, 6. Me salto
...
Es hora de ver estos atributos en acción. Ve a la página de inicio de nuestra app... asegúrate de haber
cerrado sesión... y actualiza la página.

Genial, aquí está nuestro volcado.

Todos son «false», excepto `PUBLIC_ACCESS`, que es «true». 

`PUBLIC_ACCESS` es el que se sale de la tónica. A diferencia de `IS_AUTHENTICATED_FULLY` o `IS_REMEMBERED`, no
...
¿Qué ha cambiado? Bueno, `ROLE_USER` y `IS_AUTHENTICATED` siguen siendo verdaderos, pero `IS_AUTHENTICATED_FULLY` es
ahora falso, y `IS_REMEMBERED` es ahora verdadero.
...
«¿Está este usuario autenticado mediante “recordarme”?». Significa: «¿Está este usuario autenticado al menos tanto
como un usuario con “recordarme”?».
