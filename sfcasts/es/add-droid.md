...
Puede que te estés preguntando "Espera, ¿realmente necesito crear un objeto`StarshipDroid` cada vez que quiero simplemente asignar un droide a una nave estelar?" Creo que eso es demasiado trabajo. ¿Podríamos volver a los viejos tiempos en los que bastaba con llamar a `$ship->addDroid($droid)`?

¡Sí! Esto no funcionará todavía, ¡pero eso nunca nos ha detenido antes! Carga los accesorios. ¡Ay! 
...
Ya hemos establecido el lado propietario de la relación, pero vamos a sincronizar también el otro lado. Podemos hacerlo llamando a`$droid->starshipDroids->add($starshipDroid)`. 
...
## Cascando la Persistencia

Ah, ja, un nuevo error. Éste es bastante común en Doctrine, aunque no siempre fácil de entender:
