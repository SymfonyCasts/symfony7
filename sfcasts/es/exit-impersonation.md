...
producción, sin esta barra de herramientas de depuración web, no quedaría claro que te estás
haciendo pasar por otra persona.
...
la mayoría de ellos, pero nos saltamos por alto este
...
Vuelve a la página de inicio y actualízala… ¡ya está! Ahora queda superclaro. Gran
...
Sigue en `base.html.twig`, desplázate hacia abajo hasta donde se muestran los botones de inicio de sesión y cierre de sesión
...
Y para el `href`, podemos utilizar otra función de Twig: `impersonation_exit_path()`:

[[[ code('1dee17d592') ]]]

Por cierto, hay versiones `url` de estas funciones. Hacen exactamente lo mismo,
pero generan una URL absoluta en lugar de solo la ruta.

Así que usa `impersonation_exit_path()` sin ningún argumento. Y ya está.
...
Así, si quiere, puede cambiar a otro usuario.
...
Y eso es todo sobre la suplantación de identidad. A continuación: ¡limitación de inicios de sesión!
