...
Esto resulta muy útil para que los administradores del sitio puedan depurar un problema que tenga un usuario concreto.
...
Lo único que vamos a cambiar aquí es el prefijo de la ruta. Cámbialo a `/admin/user`:

[[[ code('9ec23052ee') ]]]

Como estas rutas ahora llevan el prefijo `/admin`, nuestra configuración de `access_control` ya
las restringe solo a los administradores. Así que eso lo tenemos gratis aquí.

¡Veamos este nuevo CRUD! En la app, ve a `/admin/user`... y nos redirige a
la página de inicio de sesión. Necesitamos un administrador, así que usa `janeway@starfleet.space`, contraseña
`coffeeblack`.
...
Aquí están las opciones de configuración. « `provider` » sirve para cuando quieras usar un proveedor de usuarios
diferente por alguna razón. Recuerda que el proveedor de usuarios es cómo el sistema de seguridad
carga a los usuarios. La mayoría de las apps, como la nuestra, solo tienen un proveedor, así que podemos dejarlo
como está por defecto.

`parameter` es el parámetro de consulta de la URL que activa el cambio. También está bien
dejarlo por defecto.

Cambiar de usuario es una acción peligrosa: no quieres que
cualquier usuario pueda hacerse pasar por un administrador de tu sitio. Así que… por defecto, esta función
está restringida a un rol concreto: `ROLE_ALLOWED_TO_SWITCH`. Cualquiera que no tenga
...
es nuestra cuenta de usuario original. Y tenemos este enlace especial «Salir de la suplantación de identidad».
