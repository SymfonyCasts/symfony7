# Suplantación de identidad de usuarios con `switch_user`
...
las restringe solo a los administradores. Así que eso lo tenemos ya hecho.

¡Veamos este nuevo CRUD! En la app, ve a `/admin/user`... y nos redirige a
la página de inicio de sesión. Necesitamos un usuario de admin, así que usa `janeway@starfleet.space`, contraseña`coffeeblack`.

Este CRUD… no tiene muy buena pinta… así que vamos a darle un toque más bonito. Abre
...
Desplázate hacia arriba... hasta que encuentres la sección « `switch_user` »... aquí está.

Aquí están las opciones de configuración. « `provider` » sirve si quieres usar un proveedor de usuarios
diferente por alguna razón. Recuerda que el proveedor de usuarios es la forma en que el sistema de seguridad
...
cualquier usuario pueda hacerse pasar por un administrador de tu sitio. Así que… por defecto, esta función
...
Es hora de configurarlo. Primero, ve a `config/packages/security.yaml`. En la sección « `role_hierarchy` », bajo
«`ROLE_ADMIN` », añade `ROLE_ALLOWED_TO_SWITCH`. Ahora nuestros administradores tendrán este rol.
...
Quiero cambiar una cosa: `target_route`. Que te redirijan a `app_homepage`.

Si no lo configuras, por defecto se conectarán desde la página en la que
se encuentren en ese momento. Y ya te puedes imaginar: si cambiamos a Picard en esta página, él no
...
Ahora vamos a configurar ese botón. Vuelve a la plantilla de índice y busca el enlace «cambiar a»
... aquí está. Borra `href`. ¡Hay una función de Twig muy útil que genera
...
Aquí hay incluso un poco más de información. Nos muestra quién está suplantando la identidad:
esa es nuestra cuenta de usuario original. Y tenemos este enlace especial «Salir de la suplantación de identidad».
