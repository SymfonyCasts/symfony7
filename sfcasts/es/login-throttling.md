...
Es hora de hablar de la limitación de intentos de inicio de sesión: una forma de restringir el número de intentos fallidos.
...
en nuestro firewall « `main` », añade `login_throttling: true`.

Ahora actualiza la página de inicio de sesión... ¡y nos sale un error! La limitación de accesos requiere el componente de limitación de
velocidad de Symfony. Copia el enlace `composer require` del mensaje de error, ve a
tu terminal, añádele el prefijo `symfony`... y pega:
...
## Hacer que la limitación de accesos funcione localmente

De vuelta en nuestro IDE, hay otra cosa que cambiar. Por defecto, el limitador de tasa utiliza
...
En general, no te conviene esto: que la limitación de inicios de sesión te estorbe durante
...
Las dos opciones más importantes son « `max_attempts` » y « `interval` ». Como te decía, el
valor por defecto es de cinco intentos en un minuto, y así es como puedes ajustarlo.

Ahora, así es como funciona realmente el limitador de inicios de sesión por defecto: crea dos limitadores de frecuencia.
...
`storage_service` te permite anular por completo el sistema de almacenamiento del limitador de velocidad.

Y, por último, justo en la parte superior, « `limiter` » es donde puedes pasar tu propio servicio y
tomar el control por completo. Si usas esto, ninguna de las otras opciones tiene sentido.
Como ves en el comentario, tiene que ser un servicio que implemente`RequestRateLimiterInterface`.
...
sus empleados se conecten a Internet a través de la misma IP. Así que no restrinjas
demasiado esto. Imagina que todo el mundo inicia sesión a las nueve de la mañana y que un montón
...
Una cosa más que debes tener en cuenta: esto es una limitación de velocidad a nivel de aplicación. Cada
petición sigue llegando a tu app y sigue consumiendo recursos del servidor, incluso cuando se
limita. Así que no lo consideres una protección real contra ataques DDoS. Para eso, necesitas algo
