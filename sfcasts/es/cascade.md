...
> `Starship.droids` que no estaba configurada para operaciones de "persistencia en cascada
> para la entidad `StarshipDroid`.

Déjame que te lo traduzca:

> Oye, estás guardando este `Starship` y tiene un `StarshipDroid`
> adjunta a él. Eso es genial, pero te olvidaste de decirme que persistiera la > entidad  
...
Elimina todo el código manual y vuelve a poner la propiedad`droids` en `StarshipFactory`. 
...
Entre bastidores, Foundry está llamando a `addDroid()` en cada `Starship` para cada `droid`. Y acabamos de comprobar que `addDroid()` vuelve a funcionar.
...
Pero, ¿y si quieres añadir un `droid` a un `starship` y controlar la propiedad`assignedAt`? Añade un argumento para `addDroid()`en `Starship`: un `DateTimeImmutable`.

Que sea opcional para mantener la flexibilidad. Entonces, después de crear el`StarshipDroid`, establece el `assignedAt` si lo hemos pasado.
