...
Ahora, si compruebas tus migraciones, verás sólo la nueva que hemos creado: He limpiado las migraciones antiguas del curso pasado, así que ésta es toda `StarshipPart`. Ejecútala con:

```terminal
symfony console doctrine:migrations:migrate
```

¡La tabla está en la base de datos! Pero hay dos campos que me gusta añadir a todas mis entidades: `createdAt` y `updatedAt`. Puedes verlos dentro de `Starship`, debajo de `TimestampableEntity`. Cópialo, y pégalo justo encima de `StarshipPart`. Ambas propiedades se establecen automáticamente gracias a una biblioteca que instalamos en el último tutorial. Y como hemos añadido dos campos nuevos, ¡necesitamos una migración!
