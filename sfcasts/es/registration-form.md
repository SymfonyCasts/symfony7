...
Para terminar este curso, vamos a añadir una forma de que los nuevos usuarios se registren en la app. Y una forma bastante habitual
...
lo iniciará sesión automáticamente, así que podremos ver cómo funciona.
...
Así que, durante el proceso de validación, antes de guardar el usuario, esto comprueba si
...
Lo único que quiero añadir es nuestro campo « `name` »: `->add('name')`:
...
lo que determina qué generador de contraseñas usar— y la contraseña sin cifrar.
Devuelve la contraseña con el hash, así que la asignamos al usuario.

Ahora ya tenemos un objeto de usuario totalmente válido.

Lo guardamos y lo actualizamos en la base de datos.

A continuación es donde harías cualquier otra cosa que quieras, como añadir un mensaje flash.

Por último, devolvemos `$security->login()`: ese es el sistema de inicio de sesión programático.
Pasa los nombres de nuestro autenticador y del cortafuegos… pero esos se pueden
...
`$security->login()` sin devolverlo y, a continuación, devolver tu propia respuesta —quizá un
redireccionamiento a algún sitio—.
...
Esa plantilla es nuestro último archivo nuevo, así que ve directamente a ella.

El generador ha creado un formulario que muestra cada campo por separado. Lo único que quiero
hacer es mostrar también nuestro campo `name`. Duplica el del correo electrónico... y cámbialo por
`name`:

[[[ code('ce3165cfa8') ]]]

Ahora, si miras la página de inicio, todavía no se ve ningún cambio. Lo que quiero es un
botón «Registrarse» junto al botón de inicio de sesión, y claro, solo si aún no
has iniciado sesión.

Abre `templates/base.html.twig` y desplázate hacia abajo hasta que encuentres esa gran instrucción `if` para nuestros enlaces. Tenemos el enlace de suplantación, el botón de cerrar sesión y este de iniciar sesión.
...
En `register.html.twig`, voy a pegar una versión más bonita… La puedes encontrar en el script de abajo:
...
Hay otra restricción que hay que comprobar: esa de « `#[UniqueEntity]` » en el correo. Ya
tenemos un usuario en la base de datos con `picard@enterprise.space` como correo, así que introduce
eso y regístrate...
...
Ahora registra a un usuario nuevo de verdad: correo `natasha@enterprise.space`, nombre «Natasha Yar» y
...
Me encantaría que lo comentáramos en los comentarios de abajo. ¿Es esto un problema de seguridad? ¿Cómo podríamos
