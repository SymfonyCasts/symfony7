...
Cada campo de formulario tiene un ajuste `priority` - el valor por defecto es 0. Si añadimos`priority` al campo `Starship` y lo establecemos en 10, Symfony renderizará ese campo antes:
...
Después, añade un poco de HTML para crear una cuadrícula utilizando las clases CSS de Tailwind:`grid`, `grid-cols-2` y `gap-4`. Dentro, renderiza de nuevo `form_row()` para`form.name` y `form.price`:

[[[ code('8e8ceb18ac') ]]]

No te olvides de los campos restantes antes del botón codificado: simplemente añade`form_rest()` y pasa la variable `form` para renderizar los campos que aún no hayamos renderizado:
...
> Cualquier cosa que quieras añadir después de que se hayan mostrado todos los campos, pero antes de
> del cierre `</form>`
