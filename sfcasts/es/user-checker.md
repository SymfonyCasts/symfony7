...
sesión. Y si ya han iniciado sesión, se les cerrará la sesión inmediatamente.
...
Cámbialo a « `datetime_immutable` ». ¿Puede este campo ser « `null` »? ¡Sí! Porque recuerda: un
valor «`null` » significa que están activadas. Y… eso es todo.
...
para que sea más fácil saber si están activados o desactivados:`public function isEnabled(): bool` y, dentro, `return null === $this->disabledAt;`:
...
pero usar estados es más reutilizable y, en mi opinión, más fácil de leer.
...
¡Y para eso precisamente sirve un servicio de « `UserChecker` »!
...
`DisabledException` es una excepción de estado de cuenta. Si la analizas, verás
...
excepciones listas para usar. `CredentialsExpiredException`, por ejemplo, se podría lanzar si
tienes un sistema que caduca las credenciales a los 30 o 60 días. También están
...
`config/packages/security.yaml` y, bajo nuestro cortafuegos « `main` », añade
...
y se les desactiva durante la sesión? A continuación: vamos a asegurarnos de que un usuario desactivado
se cierre la sesión en cuanto se le desactive.
