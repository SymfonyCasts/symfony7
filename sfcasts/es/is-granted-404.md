...
¡Hola, amigos! Bienvenidos a «Seguridad en Symfony: ¡Un paso más allá!»
...
Aquí tienes nuestro Starshop. Si has seguido el curso «Conceptos básicos de seguridad», ya te resultará
familiar: tenemos un enlace de registro, un enlace de inicio de sesión y algunos usuarios predefinidos en nuestros
fixtures. Abre `src/Story/AppStory.php` para verlos. Tenemos a`picard@enterprise.space`, con contraseña `makeitso` —un capitán que tiene su propia nave espacial— y a`janeway@starfleet.space`, con contraseña `coffeeblack`, que es administrador. También creamos un montón de
naves espaciales.
...
estamos en modo anónimo. También vemos un 404. Si se hubiera devuelto el 403 por defecto, nos habrían
...
En cambio: un 404. Nadie que no esté autorizado a editar esta nave espacial puede siquiera saber que existe.

## Las otras opciones de « `IsGranted` »

Hay algunos otros parámetros relacionados con las excepciones en este atributo. Echa un vistazo a
