...
Actualiza nuestra página de inicio y haz clic en el primer carguero. Este volcado nos muestra esta instancia`Freighter`. Vuelve a la página de inicio y haz clic en el último carguero minero. Ahora el volcado nos muestra una instancia `MiningFreighter`. Aunque hemos indicado`Starship`, Doctrine sabe que debe instanciar la clase de entidad correcta.
...
Preparemos la plantilla de este controlador. El bundle creador creó este archivo`index.html.twig` en nuestro directorio `templates/starship`. Cámbiale el nombre a `show.html.twig`. Ábrelo y sustituye el bloque de título por `{{ ship.name }}`:
...
Vuelve a `StarshipController::show()`, elimina el `dd()`, cambia la plantilla a`starship/show.html.twig`, y pasa `ship` como parámetro.
