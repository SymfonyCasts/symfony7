...
Así que en nuestras plantillas de Twig usábamos esas comprobaciones de `is_granted()`, pero en este caso,
solo ocultan los enlaces. En realidad, no nos impiden seguir
esos enlaces. Para hacerlo, tenemos que denegar el acceso basándonos en esos mismos permisos
...
Pero para `new()`, nuestra regla era que tienes que ser administrador. Así que añade`#[IsGranted('ROLE_ADMIN')]`:
...
A ver cómo queda eso ahora mismo. Cierra sesión de Picard e inicia sesión como Janeway:
`janeway@starfleet.space`, contraseña `coffeeblack`.

Ahora ve a la administración de naves estelares. Vemos el botón «Crear nueva» —eso está bien— y
podemos hacer clic en él para ir a la página. Perfecto. Pero no vemos las opciones de edición
para ninguna de las naves estelares.
...
Ahora, quizá se te ocurra usar el servicio auxiliar `Security` del que hablamos antes... pero
...
Una última cosita que quiero enseñarte. En una app compleja, te puedes imaginar
que hay cientos de «voters», y que se llama a ` `supports()` ` en todos ellos para cada
...
Necesitamos ese ` `true` ` porque tienes que pasarlo cuando compruebas una clase como cadena
con ` `is_a()``.

Esto nos va a dar un pequeño aumento de rendimiento.

Ahora sobrescribe `supportsAttribute()`. Toma como argumento la `$attribute` que se está comprobando.
Copia la lógica para esto de `supports()` más arriba...
