...
la nave de otra persona. ¡Esto no está bien!
...
esos enlaces. Para hacerlo, tenemos que denegar el acceso basándonos en esos mismos permisos directamente
dentro de `StarshipAdminController`.
...
Ahora ve a la administración de naves espaciales. Vemos el botón «Crear nueva» —eso está bien— y
podemos hacer clic en él para ir a la página. Perfecto. Pero no vemos las opciones de edición
para ninguna de las naves espaciales.
...
en realidad no deberías hacerlo aquí. Veremos por qué en un momento.
...
puede que tengas un token diferente. Esto puede pasar en algunos casos extremos, así que es mejor
...
Así que escribe ` `return is_a($subjectType, Starship::class, true)``. Necesitamos ese ` `true`` 
porque tienes que pasarlo cuando compruebas una clase como cadena con ` `is_a()``.
