...
Well, well, well. It looks like *both* entities got an
update! In `StarshipPart`, we have a new `starship` property.
But instead of `ORM\Column`, it's `ORM\ManyToOne`and its value will be a
`Starship` *object*. We also have
...
`removePart()` methods. These will come in handy when you work
...
we need, but it's not super important: `ArrayCollection` is an object
looks and acts like an array, meaning we can `foreach` over it or do other array-like
things
