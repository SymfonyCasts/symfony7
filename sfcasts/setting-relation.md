...
`StarshipPart` belongs to this `Starship`"? So far, we've been working
in `AppFixtures` with Foundry. We'll come back to Foundry in a
...
Pff, try to load the fixtures anyway. Head
...
Why is that column required? In `StarshipPart` the
...
`$part->setStarship($starship)`. That's it. With Doctrine, we're not setting
some `starship_id` property or even
...
happily linked to `Starship` id 75. Look that up:
...
Here's the takeaway: with Doctrine relations,
