...
The query parameters look a bit crazy. Instead of the simple `query=legacy`, we see
`parts_search[query]=legacy` - those `%5B` and `%5D`'s are just encoded `[]`.

What's up with that? By default, Symfony forms submit data as nested arrays, keyed
by the form name. This helps avoid name collisions when you've got multiple forms
on the same page. It's a smart move, but for our search form, it's overkill, and
let's be honest, a bit ugly.
...
`query` field, add another parameter called `required` and set it to
`false`.
...
Add our usual form submission check. `if ($searchForm->isSubmitted() && $searchForm->isValid())`.
The `query` field is considered unmapped because the form isn't associated with an object.
Inside the if, set the `$query` variable to `$searchForm->get('query')->getData()`. This grabs
the query field and retrieves its submitted value.

***TIP
To grab all the form data at once, as an array, you can use `$searchForm->getData()`. 
***
...
let's be a bit more semantic and switch it to a special `SearchType`. Back in
the browser, refresh the page again. Nothing changed at first glance, but if you
start typing - there's a handy little X button that lets you easily clear
the input.

## Enabling Turbo on the Website

If you recall, I disabled Turbo at the start of this course to keep things
simple. But now it's time to unleash its power. Open `assets/app.js` and uncomment
