...
# 9. CRUD operations
- Let's speed up our form generation for CRUD operations on entities
  by leveraging MakerBundle again
- CRUD operations are operations like Create/Read/Update/Delete
- Go to the terminal
- Run `symfony console make:crud` command
- For entity, choose `Starship` this time
...
- I will tweak the route path to `#[Route('/admin/starship')]`
  to be consistent with other admin endpoints
- Now, open that new `/admin/starship` endpoint
- Ah, an error:
...
- Update the page to see it renders the list of all Starship entities
- Click on the Show link
- Aha, the same error, but we know the drill
- Do the same fix in `starship_admin/show.html.twig`
- Refresh again - it renders the list and allow us to watch the specific
  Starship details
...
- Oh, it asks a JS prompt to confirm the delete action, so nice! It prevents
  accidental clicks.
...
- Symfony Form component tries to render that when renders the whole form
- So, the proper fix should be done in form type
- Open related `StarshipType`
- The default type guessing didn't work here
- Set `status` type to `EnumType::class`
- Another error:
  > The required option "class" is missing.
- Even though you may guess the problem, let's double-check it 
  in the terminal with `symfony console debug:form EnumType`
- Aha, the class argument is the only required arg, and it should be set
  to the corresponding Enum class which is `StarshipStatusEnum::class`
- OK, add that `class` option and set to `StarshipStatusEnum::class`
- Refresh again...
- Here's out form with prefilled data
- We can edit the form and click save
- If we return back to the list page and scroll the list till the end - you
  will find the "Create new" link
- Click it - the same form behind the scene but with no data ready to create
  new starships
- OK, this generated code definitely needs some style, but it already gives us
...
- OK, refresh again
- Yes, now it looks much better styling
- Also, the "Create new" button is above the table now for faster access.
- I love it. But it was just a styling tweaks, it's still the same business
  logic generated by MakerBundle
...
- For the form theme, we should use add it to the form template as we did in `new.html.twig`
  i.e. just apply the built-in Tailwind CSS form theme
- And it will work, but wait! I don't want to do this for every new form.
  We can configure this globally in the app
- Open the terminal and execute: `symfony console config:dump twig`
- There's a `form_themes` key set to default theme
- Back to the PhpStorm
- I will comment out this `form_theme` tag setting in the template
- Copy the Tailwind CSS template name
...
- Open the new StarshipPart page - it still should contain the same styling
- The Tailwind CSS form theme is used globally now, but you can still apply
