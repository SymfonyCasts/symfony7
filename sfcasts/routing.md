...
type hint from `Starship` to `Scout`, you'll end up with a 404 error when
...
Open it up, and replace the title block with `{{ ship.name }}`. Clear out all the boilerplate
in the body block.

For the sake of simplicity, copy the contents of our teaser template and paste in
the show template's body block. In a real world app, the show template would likely
...
`starship/show.html.twig`, and pass `ship` as a parameter. Now, head back to our app
...
and change the `extends` to `starship/show.html.twig`. In `show/mining_freighter.html.twig`,
...
build the template name: `starship/show/{$ship->getType()}.html.twig`. This is
the template name for the specific starship type.
